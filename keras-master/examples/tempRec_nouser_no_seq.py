...
def readbatch():
	global staticFile,lblFile,batchsize
	...
	return static[0:i-1],y_train[0:i-1],hasmore
...
for i in range(0,10):
	...
	while True:
		print("batch",j)
		j=j+1
		staticinput , y_train,hasmore = readbatch()
		history = model.train_on_batch([staticinput ] ,y_train,accuracy=True)# nb_epoch=10, batch_size=1024, verbose=2, show_accuracy=True)
		if not hasmore :
			staticFile.close()
	#		tempFile.close()
			lblFile.close()
			break
	model.save_weights(r'\\ZW5338456\f$\temprepdiction_no_user_.model.nolstm.'+`i`)
	

model.save_weights(r'\\ZW5338456\f$\temprepdiction_no_user_.model.nolstm')
