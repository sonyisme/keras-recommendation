...
print('done complie')
model.load_weights(r'\\ZW5338456\f$\temprepdiction_no_user_.model.nolstm')

j=0
while True:
	print("batch",j)
	j=j+1
	staticinput ,hasmore = readbatch()
	ys = model.predict([staticinput ])# ,y_train,accuracy=True)# nb_epoch=10, batch_size=1024, verbose=2, show_accuracy=True)
	for y in ys:
		for yi in y:
			outfile.write("%s\t" %yi)
		outfile.write("\n")
